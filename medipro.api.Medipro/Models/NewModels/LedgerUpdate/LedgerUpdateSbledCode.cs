...
{
    public class LedgerUpdateSbledCode
    {
        public void LedgerUpdate()
        {
            StringBuilder s = new StringBuilder();

            s.Append(" create proc sp_ledger_SbledCode_Update (@sbled_code int) ");
            s.Append(" as ");
            s.Append(" begin ");
            s.Append(" begin try ");
            s.Append(" begin transaction ");
            s.Append(" update ledger set sbled_code =0 where sbled_code is null ");
